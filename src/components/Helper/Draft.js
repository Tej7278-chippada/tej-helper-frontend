...
const Helper = ({ darkMode, toggleDarkMode, unreadCount, shouldAnimate})=> {
  ...
  const [bloodDonorFilters, setBloodDonorFilters] = useState({
    bloodGroup: 'All'
  });
  ...
  }, [loading, hasMore, loadingMore, showBloodDonors]);
  ...
  const handleCategorySelect = (value) => {
    // Check if BloodDonars category is selected
    if (value === 'BloodDonars') {
      ...
      // Clear cache and fetch blood donors
      globalCache.lastCacheKey = null;
    ...
    const isService = [
      'BloodDonars', 'ParkingSpace', 'VehicleRental', 'FurnitureRental', 'Laundry', 'Events', 'Playgrounds', 'Cleaning',
...
const fetchBloodDonorsData = async () => {
  ...
  const currentCacheKey = `bloodDonors_${userLocation.latitude}_${userLocation.longitude}_${distanceRange}_${bloodDonorFilters.bloodGroup}_${searchQuery}`;
  setPosts([]);
  setPostMarkers([]);
  ...
  try {
    ...
    // Check cache for blood donors
    if (globalCache.bloodDonorsData && 
        globalCache.bloodDonorsData[currentCacheKey] &&
        Date.now() - globalCache.bloodDonorsData[currentCacheKey].timestamp < 3600000) {
      
      const cachedData = globalCache.bloodDonorsData[currentCacheKey];
      setBloodDonors(cachedData.donors);
      setTotalPosts(cachedData.totalCount);
      setLoading(false);
      return;
    }

    const response = await fetchBloodDonors(0, 12, userLocation, distanceRange, bloodDonorFilters, searchQuery);
    const donorsData = response.data.donors || [];
    const totalCount = response.data.totalCount || 0;

    // Cache the blood donors data
    if (!globalCache.bloodDonorsData) {
      globalCache.bloodDonorsData = {};
    }
    globalCache.bloodDonorsData[currentCacheKey] = {
      donors: donorsData,
...
// Function to fetch blood donor locations for map
// Function to create blood donor markers on map - FIXED
const createBloodDonorMarkers = (donorLocations) => {
  if (!mapRef.current) { // Use mapRef.current instead of map
  ...
  // Clear existing blood donor markers
  if (window.bloodDonorMarkers) {
    window.bloodDonorMarkers.forEach(marker => marker.remove());
  }
  window.bloodDonorMarkers = [];
  ...
  donorLocations.forEach(donor => {
    if (donor.location && donor.location.latitude && donor.location.longitude) {
      ...
      const marker = L.marker([donor.location.latitude, donor.location.longitude], { 
        icon: customIcon 
      }).addTo(map);
      ...
      marker.bindPopup(`
        <div style="min-width: 200px;">
          ...
          ${donor.trustLevel ? `<p style="margin: 4px 0; color: #666;">
            <strong>Trust Level:</strong> ${donor.trustLevel}/5
          </p>` : ''}
        </div>
      `);

      window.bloodDonorMarkers.push(marker);
    }
  });
};
...
const fetchBloodDonorLocationsData = async () => {
  if (!userLocation || !distanceRange || !showBloodDonors) return;

  const currentLocationsCacheKey = `bloodDonorLocations_${userLocation.latitude}_${userLocation.longitude}_${distanceRange}_${bloodDonorFilters.bloodGroup}_${searchQuery}`;

  try {
    const response = await fetchBloodDonorLocations(userLocation, distanceRange, bloodDonorFilters, searchQuery);
...
// Function to create custom marker element for blood donors
// const createCustomDonorMarkerElement = (donor) => {
//   const element = document.createElement('div');
//   element.className = 'blood-donor-marker';
//   element.innerHTML = `
//     <div class="blood-donor-marker-content">
//       <div class="blood-drop-icon">💉</div>
//       <div class="blood-group">${donor.bloodDonar.bloodGroup}</div>
//     </div>
//   `;
//   return element;
// };
...
const loadMoreBloodDonors = async () => {
  if (loadingMore || bloodDonors.length >= totalPosts) return;
  
  try {
    setLoadingMore(true);
    const response = await fetchBloodDonors(bloodDonors.length, 12, userLocation, distanceRange, bloodDonorFilters, searchQuery);
...
}, [userLocation, distanceRange, bloodDonorFilters, searchQuery, showBloodDonors]);
...
const handleDonorClick = (donor) => {
  ...
  // Fetch posts data
  useEffect(() => {
    if (!distanceRange || !userLocation || showBloodDonors) {
      setPosts([]);
      return;
    }
    ...
    const fetchData = async () => {
        ...
        try {
          setBloodDonors([]); // Clear blood donors when fetching posts
          setLoading(true);
  ...
  return (
      ...
      >
        ...
        {selectedCategory !== 'Friends' && (
          ...
          {loadingLocation || loading ? (
              <SkeletonCards/>
            ) : showBloodDonors ? ( // Check if showing blood donors
      /* BLOOD DONORS DISPLAY */
      bloodDonors.length > 0 ? (
        <Grid container spacing={isMobile ? 1.5 : 1.5}>
          {bloodDonors.map((donor, index) => (
            <Grid item xs={12} sm={6} md={4} key={`${donor._id}-${index}`} ref={index === bloodDonors.length - 3 ? lastPostRef : null}>
              <BloodDonorCard
                donor={donor} 
        ...
        <Box sx={{ 
          ...
          {!searchQuery && (
            ...
            {/* Loading More Indicator for Blood Donors */}
    {showBloodDonors && loadingMore && (
      <Box sx={{ display: 'flex', justifyContent: 'center', alignItems:'center', p: 4, gap:'1rem' }}>
        <CircularProgress size={24} />
        <Typography color='grey' variant='body2'>Loading blood donors...</Typography>
      </Box>
    )}
    ...
    {!showBloodDonors && loadingMore && (
      <Box sx={{ display: 'flex', justifyContent: 'center', alignItems:'center', p: 4, gap:'1rem' }}>
        ...
        <Typography color='grey' variant='body2'>Loading posts...</Typography>
            ...
            {/* End Message for Blood Donors */}
    {showBloodDonors && !hasMore && bloodDonors.length > 0 && (
      <Box sx={{ 
        textAlign: 'center', 
        p: 3,
        ...
        <Typography color="text.secondary">
          You've reached the end of <strong>{totalPosts} blood donors</strong> in your area
        </Typography>
    ...
    {/* End Message for Posts */}
    {!showBloodDonors && !hasMore && posts.length > 0 && (
      <Box sx={{ 
        textAlign: 'center', 
        p: 3,

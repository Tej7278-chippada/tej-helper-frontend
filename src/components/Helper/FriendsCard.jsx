...
const FriendsCard = ({ donor, onClick, darkMode }) => {
    ...
    >
      <CardContent sx={{ p: 2 }}>
        ...
        <Box sx={{ my: 1 }}>
          ...
          {(donor?.friendsProfile?.contactWay?.phone || donor?.friendsProfile?.contactWay?.email ||  donor?.friendsProfile?.contactWay?.socialMedia?.length > 0) ? (
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
              {donor.friendsProfile.contactWay.phone && <PhoneIcon fontSize="small" sx={{ color: '#0088cc' }} />}
              {donor.friendsProfile.contactWay.email && <EmailIcon fontSize="small" sx={{ color: '#E4405F' }} />}

              {donor.friendsProfile.contactWay.socialMedia.map((social, index) => {

...
const NotificationSettings = ({ open, onClose, darkMode, isMobile, socket, userId, setSnackbar }) => {
  ...
  const checkNotificationStatus = async () => {
    ...
    try {
      ...
      setDistanceRange(response.data.notificationRadius);
      setNotificationTypes({
        nearbyPosts: response.data.notificationPreferences.nearbyPosts,
        emergencies: response.data.notificationPreferences.emergencies,
        messages: response.data.notificationPreferences.messages,
        updates: response.data.notificationPreferences.updates
      })
    } catch (error) {
            ...
            <Box display="flex" alignItems="center" justifyContent="space-between">
              <Typography variant="body2" fontWeight={500}>
                {notificationsEnabled
                  ? "You're turned on the notifications!"
                  : "Would you like to get notifications?"}
              </Typography>
              <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {loadingToggle && <CircularProgress size={18} />}
                <Switch
    ...
    >
      ...
      {step === 1 && (
        <DialogActions sx={{ gap: 1 }}>
          <Button 
            onClick={handleLater}
            disabled={loading || loadingToggle}
            variant="outlined"
            sx={{ 
              borderRadius: '12px', 
              textTransform: 'none',
          ...
          <Button 
            ...
            variant="contained"
            disabled={loading || !notificationsEnabled || loadingToggle}
            disableElevation
            sx={{ 
              borderRadius: '12px',
              background: 'linear-gradient(135deg, #ff6b6b 0%, #ee5a52 100%)',
          ...
          >
            {loading ? 'Enabling...' : 'Update Preferences'}
          </Button>
        </DialogActions>
      )}
    </Dialog>
